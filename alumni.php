...
<form method="POST">
	...
	<option>All</option>
		<?php
			$sql = "SELECT DISTINCT Pledge_Class FROM Members WHERE Mem_Status = 'Alumni'";
			foreach($pdo->query($sql) as $row){
				$PClass = $row["Pledge_Class"];
				echo "<option value='$PClass' name='PledgeClass' width='100%'>$PClass</option>";
			}
		?>
	<input name="PCSubmit" type="submit" style="margin:10px;" value="Search"/>
	</select>
	...
	<option>All</option>
		...
		?>
	<input name="FamSubmit" type="submit" style="margin:10px;" value="Search"/>
	</select>
	</form>
	<br>
	...
	if(isset($_POST["NameSearch"])) {
		...
		$sql = "SELECT Mem_FName, Mem_LName, Mem_Email, Mem_Year, Mem_Family, Pledge_Class FROM Members WHERE Mem_Status = 'Alumni' && (Mem_FName LIKE '%$keyword%' OR Mem_LName LIKE '%$keyword%')";
		$q = $pdo->query($sql);
	...
	if(isset($_POST["PCSubmit"])) {
		...
		if($pc=="All")	{
			$sql = "SELECT Mem_FName, Mem_LName, Mem_Email, Mem_Year, Mem_Family, Pledge_Class FROM Members WHERE Mem_Status = 'Alumni'";
		}
		
		else {
			echo "<h3><center>" . $pc . " Class" . "</center></h3>";
			$sql = "SELECT Mem_FName, Mem_LName, Mem_Email, Mem_Year, Mem_Family, Pledge_Class FROM Members WHERE Pledge_Class = '" . $pc . "' && Mem_Status = 'Alumni'";
		}
		$q = $pdo->query($sql);
		$q->setFetchMode(PDO::FETCH_ASSOC);
	}
	...
	if(isset($_POST["FamSubmit"])) {
		$fam = $_POST["Family"];
			...
			$sql = "SELECT Mem_FName, Mem_LName, Mem_Email, Mem_Year, Mem_Family, Pledge_Class FROM Members WHERE Mem_Family = '" . $fam . "' && Mem_Status = 'Alumni'";
		$q = $pdo->query($sql);
		$q->setFetchMode(PDO::FETCH_ASSOC);
	}
	?>
